...
def procesar_texto(texto: str) -> str:
    ...
    esperando = {}
    # Si estamos en medio de un comando
    if esperando:
        cmd = esperando["cmd"]

        if cmd == "nmap_host":
            esperando["host"] = texto
        ...
        if cmd == "hydra_servicio":
            esperando["servicio"] = texto
            esperando["cmd"] = "hydra_usuario"
            return "¿Qué usuario usarás para el ataque?"

        if cmd == "hydra_usuario":
            esperando["usuario"] = texto
            esperando["cmd"] = "hydra_dicc"
            return "¿Ruta al diccionario/password list?"

        if cmd == "hydra_dicc":
            host = esperando["host"]
            servicio = esperando["servicio"]
            usuario = esperando["usuario"]
            dicc = texto
            esperando = {}
            return fuerza_bruta_hydra(host, servicio, dicc, usuario)

        if cmd == "msf_rhosts":
            esperando["rhosts"] = texto
            esperando["cmd"] = "msf_exploit"
            return "¿Exploit a usar? (ruta dentro de Metasploit)"

        if cmd == "msf_exploit":
            esperando["exploit_path"] = texto
            esperando["cmd"] = "msf_payload"
            return "¿Payload deseado?"

        if cmd == "msf_payload":
            rhosts = esperando["rhosts"]
            exploit_path = esperando["exploit_path"]
            payload = texto
            esperando = {}
            return exploit_metasploit(rhosts, exploit_path, payload)

    # Si no estamos en flujo especial
    if "escanear red" in texto:
        esperando = {"cmd": "nmap_host"}
        return "¿Cuál es el host o IP a escanear?"

    if "bruteforce" in texto or "fuerza bruta" in texto:
        esperando = {"cmd": "hydra_host"}
        return "¿Cuál es el host o IP a atacar?"

    if "usar exploit" in texto or "exploit metasploit" in texto:
        esperando = {"cmd": "msf_rhosts"}
        return "¿Cuál es la IP o rango (RHOSTS)?"

    #----------Auditoria-------------
    if "auditoría rápida" in texto:
        return auditoria_rapida()
    if "auditoría completa" in texto:
        return auditoria_completa()
    
    #----------Diagnostico Seguridad-------------
    if "diagnóstico" in texto or "seguridad" in texto:
    ...
    texto_personalizado = aplicar_personalizaciones(texto)

    # Si no entiende
    respuesta = "No entendí eso."

    if respuesta in ["No entendí eso.", "No pude entender qué deseas hacer.", "¿Puedes reformularlo?"]:
        correccion = input(f"No entendí '{texto_original}'. ¿Qué querías que hiciera? (escribe el comando correcto): ")
        if correccion:
            confirmar = input(f"¿Quieres que recuerde que '{texto_original}' significa '{correccion}'? (sí/no): ")
            if confirmar.strip().lower() == "sí":
                registrar_sinonimo(texto_original, correccion)
                # Solo una vez, no recursivo:
                return procesar_texto(correccion)

        return respuesta

    #----------Google calendar-------------
    if "eventos de hoy" in texto or "qué tengo hoy" in texto:
    ...
    if "silencia" in texto or "mutea" in texto:
        return silenciar()

    if "reanuda el sonido" in texto or "activa el sonido" in texto:
        return reanudar_sonido()

    if "pausa la música" in texto or "reproduce la música" in texto:
        return pausar_musica()

    if "siguiente canción" in texto:
        return siguiente_cancion()

    if "canción anterior" in texto:
        return anterior_cancion()

    
    #-------Control entornos digital--------
    if "abre" in texto or "abrir" in texto:
        app = texto.split("abre")[-1].strip() if "abre" in texto else texto.split("abrir")[-1].strip()
        return abrir_aplicacion(app)

    #---------Conflictos-----------------------------
    if "conflictos" in texto or "tengo coincidencias" in texto:
        conflictos = verificar_conflictos_entre_tareas_y_eventos()
        if not conflictos:
            return "Todo en orden, no hay conflictos entre tus tareas y eventos."
        else:
            respuesta = "He encontrado los siguientes conflictos:\n"
            for c in conflictos:
                respuesta += f"- Tarea urgente '{c['tarea']}' coincide con el evento '{c['evento']}' a las {c['hora']}\n"
            return respuesta

    #---------Agenda y eventos-----------------------------
    respuesta_evento = detectar_evento_y_guardar(texto)
    if respuesta_evento:
        return respuesta_evento
    if "tengo algo" in texto or "qué tengo" in texto or "eventos" in texto:
    ...
    if emocion:
        ...
        if emocion == "tristeza":
            return "Siento que estés pasando por un momento difícil. Estoy aquí para ayudarte en lo que necesites."
        elif emocion == "alegría":
            return "¡Me alegra mucho escuchar eso! Si necesitas algo, estaré encantado de ayudarte."
        elif emocion == "estrés":
            return "El estrés puede ser pesado. ¿Quieres que te ayude a organizar tus pendientes o a desconectar un rato?"
        elif emocion == "enojo":
            return "Entiendo que estés molesto. ¿Quieres que te ayude a resolver algo o simplemente desahogarte?"

    #-------------Tareas------------------------
    if "tareas pendientes" in texto or "qué tareas" in texto:
        if "urgentes" in texto:
            return obtener_tareas_pendientes("urgente")
        elif "bajas" in texto:
            return obtener_tareas_pendientes("baja")
        return obtener_tareas_pendientes()

    if "agrega tarea" in texto or "añade tarea" in texto:
        agregar_tarea_pendiente(texto)
        return "He añadido la nueva tarea según tus indicaciones."

    if "completada la tarea" in texto:
        tarea = texto.split("completada la tarea")[-1].strip()
        return completar_tarea(tarea)


    #------------------------Camara-----------------
    if any(frase in texto.lower() for frase in ["ver la cámara", "ver camara", "mostrar cámara", "mostrar camara", "activar cámara"]):
        return mostrar_video_en_vivo()

    if "ver cámara" in texto or "hay alguien" in texto or "detecta rostro" in texto:
        return detectar_rostros_webcam()
    
    if "ver la cámara" in texto.lower() or "ver camara" in texto.lower() or "mostrar cámara" in texto.lower():
        return mostrar_video_en_vivo()  # Esta función usará OpenCV para mostrar la webcam

    if "ver lo que se ve" in texto.lower() and obtener_conocimiento("camara_autorizada"):
        return detectar_rostros_webcam()
    
    if "detectar objetos" in texto.lower() or "ver objetos" in texto.lower():
        return detectar_objetos_yolo()

    #-------------------------------------------

    if "analiza imagen" in texto.lower():
        return seleccionar_y_analizar_imagen()
    
    #---------------Base de datos de conocimiento personal----------------
    # Aprender el nombre
    if "me llamo" in texto:
        match = re.search(r"me llamo ([\w\s]+)", texto)
        if match:
    ...
    # Aprender la ciudad
    if "mi ciudad es" in texto:
        match = re.search(r"mi ciudad es ([\w\s]+)", texto)
        if match:
    ...
    # Aprender intereses
    if "me interesa" in texto or "me interesan" in texto:
        match = re.search(r"me interesa[n]* ([\w\s,]+)", texto)
        if match:
    ...
    nombre_respuesta = detectar_y_guardar_nombre(texto)
    if nombre_respuesta:
        return nombre_respuesta
    
    # Si ya sabe el nombre, úsalo en la respuesta (solo de ejemplo)
    nombre = obtener_conocimiento("nombre")

    if nombre and texto in ["hola", "hey", "buenos días"]:
        return f"Hola de nuevo, {nombre}. ¿En qué puedo ayudarte hoy?"
    
    if "sabes cómo me llamo" in texto or "sabes mi nombre" in texto:
        nombre = obtener_conocimiento("nombre")
        if nombre:
    ...
    if nombre and any(s in texto.lower() for s in ["hola", "buenas", "hey"]):
        return f"Hola de nuevo, {nombre}. ¿En qué puedo ayudarte hoy?"


    #----------------Analisis de datos--------------------------
    if any(p in texto for p in ["analiza archivo", "lee archivo", "procesa archivo"]):
    ...
    if "grafica columna" in texto:
        match = re.search(r"grafica columna ([\w\s]+)", texto)
        if match:
    ...
    if "limpiar conversación" in texto.lower():
        conversation_turns[:] = [conversation_turns[0]]
        ultima_respuesta = ""
        return "Conversación reiniciada."
    if es_pregunta_fecha_hora(texto): return responder_fecha_hora()
    if filtrar_lenguaje(texto): return "Lo siento, no puedo procesar ese lenguaje."
